...
$id = $_GET['id'];
$analyzerDataQuery = "SELECT courses.title title, courses.inserted_date inserted_date, 
CONCAT(user.first_name, ' ', user.last_name) fullname, courses.content content, courses.image image 
FROM courses INNER JOIN users user ON courses.inserted_by = user.id WHERE courses.id = {$id}";
...
<head>
    ...
    <style>.main_menu_area{overflow:visible; background:#0D0E0F; min-height:50px}
...
.main_menu_area ul li a:hover{background:#CF0000}</style>
</head>

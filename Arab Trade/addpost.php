...
if (isset($_POST['delete'])) {
  ...
  $sql =  "UPDATE post SET status = 0 WHERE id = $postId AND inserted_by = {$_SESSION['id']}";
...
$id = $_GET['id'] ?? null;
if (isset($id)) {
  $postQuery = "SELECT * FROM post WHERE id = {$id} AND status = 1";
...
if (isset($_POST['update-post'])) {
  ...
  $sql = "UPDATE post SET title = ?, content = ?, updated_by = ?, updated_date = NOW(), image = ? WHERE id = $id";
...
if (isset($_POST['new-post'])) {
  ...
  $sql = "INSERT INTO post (title, content, inserted_by, inserted_date, status, image) VALUES (?, ?, ?, NOW(), 1, ?)";
  $user = $connection->prepare($sql);
  $user->execute([$title, $content, $_SESSION['id'], $fileNemeNew]);
...
$analyzerDataQuery = "SELECT * FROM post WHERE inserted_by = {$_SESSION['id']} AND status = 1 ORDER BY inserted_date DESC";
...
<body>
  <div class="body_wrapper">
    <div class="center">
      ...
      <div class="newsDiv">
        <div class="titlediv">
          <h2 style="letter-spacing: 2px;"><strong>Add Post</strong></h2>
          <hr>
        </div>

        <form action="" id="analyzer-form" method="post" enctype="multipart/form-data">
          <div class="row">
            <div class="col-lg-6">
